	...
	$(document).ready( function() {
		...
		if (localStorage.getItem('CURRENTFOLDER') == null){
			console.log('No current folder set. Creating dummy files')
			var currentFolder = new Folder('Home', 'Owner', folderCount);
			folderCount += 1;

			var cancerFolder = new Folder('Cancer', 'Owner', folderCount);

			folderCount += 1;

			var cancerFolder1 = new Folder('Breast Cancer', 'Owner', folderCount);
			folderCount += 1;

			var cancerFolder2 = new Folder('Lung Cancer', 'Owner', folderCount);
			folderCount += 1;

			cancerFolder.addFolder(cancerFolder1);
			cancerFolder.addFolder(cancerFolder2);

			var HIVFolder = new Folder('HIV', 'Owner', folderCount);
			folderCount += 1

			var thyroidFolder = new Folder('Thyroid', 'Owner', folderCount);
			folderCount += 1

			var titrationFile = new Note('Titration', 'Owner', fileCount);
			fileCount += 1

			var PCRFile = new Note('PCR ', 'Owner', fileCount);
			fileCount += 1

			var PCRTemplate = new Template('PCR', 'Owner', fileCount);
			templateCount += 1

			currentFolder.addFolder(cancerFolder);
			currentFolder.addFolder(HIVFolder);
			currentFolder.addFolder(thyroidFolder);

			currentFolder.addFile(titrationFile);
			currentFolder.addFile(PCRFile);

			currentFolder.addTemplate(PCRTemplate);
		}
		else{
		...
		// currentFolder = Storage.getHomeFolder()
		...
		for (var key in currentFolder.templates) {
			newTemplate = currentFolder.templates[key];
			console.log(currentFolder.templates[key]);
